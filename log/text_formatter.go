...
type TextFormatter struct {
	...
	// MaxMessageSise defines max size of the entry message to be printed,
	...
	if f.isColored() {
		...
		for _, key := range fixedKeys {
			...
			case key == f.FieldMap.resolve(FieldKeyMsg):
				if f.MaxMessageSize > 0 && len(entry.Message) > int(f.MaxMessageSize) {
					value = entry.Message[:f.MaxMessageSize] + "..."
				} else {
					value = entry.Message
				}
